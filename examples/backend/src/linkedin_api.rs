...
async fn exchange_code_for_token() -> Result<Token, ClientErr> {
    ...
    let built_url = RedirectURL::new(config)
        .token_exchange_url(code.to_string())
        .full_url;
...
async fn post_by_share_article() -> Result<String, ClientErr> {
    ...
    let token = "xxxxxxxxx";
    let post_message =
        "What we love about nature is that all of its  act are naturally and amazing "; //  Our message
    ...
    let resp = Client::default()
        .share(person_id.to_string(), token.to_string())
        .post_article(post_message.to_string(), media_url)
        .await?;
    Ok(resp)
}
...
pub async fn linkedin_video_publish_by_id() -> Result<String, ClientErr> {
    ...
    let resp = Client::default()
        .publish_video(person_id.to_string(), token.to_string())
        .publish_media_by_id(
            post_message.to_string(),
            media,
            "the space of time ".to_string(),
        )
        .await?;
    Ok(resp)
}
...
pub async fn linkedin_video_status() -> Result<String, ClientErr> {
    ...
    let resp = Client::default()
        .video_upload(person_id.to_string(), token.to_string())
        .upload_status(media)
        .await?;
    Ok("".to_string())
}
...
pub async fn linkedin_upload_and_share_video() -> Result<String, ClientErr> {
    ...
    let post_description = " In the Matrix   where every is in infinite loop,  breaking the loop is all that matters  .......  the ' UNIMATRIX ' ".to_string();
    let video_file = File::open("/home/azibodusio/Downloads/marix.mp4").unwrap();

    let video_title = "the video tile".to_string(); // This will display at the bottom of the video
    let thumbnail = None; // or File
    let caption = None; // or File
    let video_purpose = "FEED_VIDEO".to_string(); // or VIDEO_AD

    let resp = Client::default()
        .publish_video(person_id.to_string(), token.to_string())
        .publish_video_upload(
            post_description,
            video_title,
            video_file,
            thumbnail,
