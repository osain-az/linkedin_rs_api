...
use crate::video::utils::VideoUploadStatus;
use serde_json::json;
use std::fs::File;
...
impl PostAPI {
    ...
    pub async fn publish_media_by_id(
        &self,
    ...
    pub async fn publish_video_upload(
        &self,
        post_desciption: String,
        video_title: String,
        video: File,
        ...
        let upload_resp = video_api
            .video_upload_handeler(video, video_caption, video_thumbnail, video_purpose)
            .await?;
        ...
        let person_id = self.person_id.clone();
        let url = &self.base_url.clone();
        let data = create_data(post_desciption, person_id, video_title, video_id);
